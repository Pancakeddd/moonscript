local Transformer
Transformer = require("moonscriptplus.transform.transformer").Transformer
...
local NameProxy
NameProxy = require("moonscriptplus.transform.names").NameProxy
local Accumulator, default_accumulator
do
  local _obj_0 = require("moonscriptplus.transform.accumulator")
  Accumulator, default_accumulator = _obj_0.Accumulator, _obj_0.default_accumulator
end
local lua_keywords
lua_keywords = require("moonscriptplus.data").lua_keywords
...
construct_comprehension = require("moonscriptplus.transform.comprehension").construct_comprehension
local insert
insert = table.insert
  ...
  chain = function(self, node)
    if self.macros:is_macro(node[2][2]) then
      ...
      local builder_functions = {
        ...
        tostr = function(x)
          rend = self:line(x):render(self:lines()):flatten()
          code = ""
          for _index_0 = 1, #rend do
            local f = rend[_index_0]
            code = code .. f
