...
local Stack
Stack = require("moonscriptplus.data").Stack
...
local wrap_env
wrap_env = require("moonscriptplus.parse.env").wrap_env
...
do
  local _obj_0 = require("moonscriptplus.parse.literals")
...
do
  local _obj_0 = require("moonscriptplus.parse.util")
...
local build_grammar = wrap_env(debug_grammar, function(root)
  ...
  local g = P({
    ...
    Statement = pos(Import + While + With + For + ForEach + Switch + Return + Insert + Macro + Local + Export + BreakLoop + Ct(ExpList) * (Update + Assign) ^ -1 / format_assign) * Space * ((key("if") * Exp * (key("else") * Exp) ^ -1 * Space / mark("if") + key("unless") * Exp / mark("unless") + CompInner / mark("comprehension")) * Space) ^ -1 / wrap_decorator,
    ...
    QuoteInsert = Space * C(sym("``") + sym("`")) * ChainValue / mark("quoteinsert"),
    ...
    SimpleValue = If + Unless + Quote + Switch + With + ClassDecl + ForEach + For + While + Cmt(Do, check_do) + sym("-") * -SomeSpace * Exp / mark("minus") + sym("#") * Exp / mark("length") + sym("~") * Exp / mark("bitnot") + key("not") * Exp / mark("not") + sym("?") * Exp / mark("nilcheck") + TblComprehension + TableLit + Comprehension + FunLit + Num,
    ChainValue = (Chain + Callable) * Ct(InvokeArgs ^ -1) / join_chain,
    Value = pos((ChainValue + SimpleValue + String) * Pipe / mark("pipe") + SimpleValue + Ct(KeyValueList) / mark("table") + QuoteInsert + ChainValue + String),
    SliceValue = Exp,
